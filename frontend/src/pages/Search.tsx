...
const Search = () => {
    ...
    // whenever the user has loaded the search page or searchParams change this query will run
    ...
    const handleHotelTypeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSelectedHotelTypes(prevHotels => event.target.checked ? [...selectedHotelTypes, hotelType] : prevHotels.filter(hType => hType !== hotelType))
    }

    const handleFacilityChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const facility = event.target.value

        setSelectedFacilities(prevFacilities => event.target.checked ? [...selectedFacilities, facility] : prevFacilities.filter(hotelFacility => hotelFacility !== facility))
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-5">
            <div className="rounded-lg border border-slate-300 p-5 h-fit sticky top-10">
                <div className="space-y-5">
                    <h3 className="text-lg font-semibold border-b borer-slate-300 p-5">Filter by:</h3>
